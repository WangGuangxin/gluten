...
class ParquetEncryptionDetectionSuite extends GlutenQueryTest {
  ...
  private def isEncrypted(fileStatus: LocatedFileStatus): Boolean = {
    try {
      ...
      case _: ParquetCryptoRuntimeException =>
        // An encrypted footer without keys is still sufficient evidence that the file is encrypted.
        true
    }
  }
  ...
  test("Detect encrypted Parquet with encrypted footer") {
    withTempDir {
      tempDir =>
        ...
        assertTrue(isEncrypted(fileStatus))
    }
  }
  ...
  test("Detect plain (unencrypted) Parquet file") {
    withTempDir {
      tempDir =>
        ...
        assertFalse(isEncrypted(fileStatus))
    }
  }
}

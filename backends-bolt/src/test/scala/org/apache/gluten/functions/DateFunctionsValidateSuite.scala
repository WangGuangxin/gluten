...
abstract class DateFunctionsValidateSuite extends FunctionsValidateSuite {
  ...
  test("date_format") {
    withTempPath {
      path =>
        ...
        runQueryAndCompare("SELECT date_format(c0, 'yyyy') FROM t")(_ => ())
    }
  }

  ignore("date_part, extract, weekofyear") {
    withTempPath {
      path =>
        Seq(
  ...
  test("from_utc_timestamp") {
    withTempPath {
      path =>
        ...
        runQueryAndCompare("SELECT from_utc_timestamp(a, b) from view")(_ => ())
    }
  }
  ...
  test("timestamp_micros") {
    runQueryAndCompare("select timestamp_micros(l_orderkey) from lineitem")(_ => ())
  }

  test("timestamp_millis") {
    runQueryAndCompare("select timestamp_millis(l_orderkey) from lineitem")(_ => ())
  }
  ...
  testWithMinSparkVersion("timestampadd", "3.3") {
    withTempPath {
      path =>
        ...
        runQueryAndCompare("select timestampadd(day, quantity, ts) from time")(_ => ())
    }
  }
  ...
  testWithMinSparkVersion("timestampdiff", "3.3") {
    withTempPath {
      path =>
        ...
        spark.read.parquet(path.getCanonicalPath).createOrReplaceTempView("time")
        runQueryAndCompare("select timestampdiff(SECOND, t1, t2) from time")(_ => ())
    }
  }
  ...
  test("to_date") {
    withTempPath {
      path =>
        ...
        spark.read.parquet(path.getCanonicalPath).createOrReplaceTempView("time")
        runQueryAndCompare("select to_date(t, 'yyyy-MM') from time")(_ => ())
    }
  }
  ...
  test("to_timestamp") {
    withTempPath {
      path =>
        ...
        spark.read.parquet(path.getCanonicalPath).createOrReplaceTempView("time")
        runQueryAndCompare("select to_timestamp(t, 'yyyy-MM') from time")(_ => ())
    }
  }
  ...
  test("to_utc_timestamp") {
    withTempPath {
      path =>
        ...
        runQueryAndCompare("SELECT to_utc_timestamp(a, b) from view")(_ => ())
    }
  }
  ...
  test("unix_micros") {
    withTempPath {
      path =>
        ...
        spark.read.parquet(path.getCanonicalPath).createOrReplaceTempView("time")
        runQueryAndCompare("select unix_micros(t) from time")(_ => ())
    }
  }
  ...
  test("unix_millis") {
    withTempPath {
      path =>
        ...
        spark.read.parquet(path.getCanonicalPath).createOrReplaceTempView("time")
        runQueryAndCompare("select unix_millis(t) from time")(_ => ())
    }
  }
  ...
  test("unix_seconds") {
    withTempPath {
      path =>
        ...
        spark.read.parquet(path.getCanonicalPath).createOrReplaceTempView("view")
        runQueryAndCompare("select unix_seconds(t) from view")(_ => ())
    }
  }
      ...
      path =>
        ...
        runQueryAndCompare(
          "SELECT a, window.start, window.end, count(*) as cnt FROM" +
            " string_timestamp GROUP by a, window(b, '5 minutes') ORDER BY a, start;")(_ => ())
    }
  }
  ...
  test("unix_timestamp with timestamp and format - no fallback") {
    withTempPath {
      path =>
        ...
        runQueryAndCompare("SELECT unix_timestamp(ts, fmt) FROM unix_timestamp_test")(_ => ())
    }
  }

  ignore("months_between") {
    withTempPath {
      path =>

...
class MiscOperatorSuite extends BoltWholeStageTransformerSuite with AdaptiveSparkPlanHelper {
  ...
  override protected def sparkConf: SparkConf = {
    super.sparkConf
      ...
      .set(GlutenConfig.COLUMNAR_FILESCAN_ENABLED.key, "true")
  }
  ...
  ignore("orc scan") {
    val df = spark.read
      .format("orc")
      .load("cpp/bolt/benchmarks/data/bm_lineitem/orc/lineitem.orc")
  ...
  ignore("Fallback on timestamp column filter") {
    withTable("ts") {
  ...
  test("cast date to timestamp with timezone") {
    ...
    try {
      ...
      // This query can be constant-folded to LocalTableScan on Spark 3.5, so
      // offload shape is not the contract here. Validate timezone semantics only.
      checkAnswer(dfInLA, Seq(Row(Timestamp.valueOf("2023-01-02 00:00:00"))))
      checkAnswer(dfInSH, Seq(Row(Timestamp.valueOf("2023-01-02 00:00:00"))))
    } finally {
      sql(s"SET spark.sql.session.timeZone = $originalTimeZone")
    }
  }

  ignore("cast timestamp to date") {
  ...
  ignore("timestamp broadcast join") {

...
abstract class BoltAggregateFunctionsSuite extends BoltWholeStageTransformerSuite {
  private val localTzDir = "/tmp/gluten-tzdata/tzdata/zoneinfo"
  ...
  override protected def sparkConf: SparkConf = {
    super.sparkConf
      ...
      .set("spark.driver.extraJavaOptions", s"-DTZDIR=$localTzDir")
      .set("spark.executorEnv.TZDIR", localTzDir)
  ...
  test("distinct functions") {
    ...
    runQueryAndCompare(
      "SELECT array_sort(collect_list(DISTINCT n_name)), count(*), " +
        "array_sort(collect_list(n_name)) FROM nation") {
      df =>
        {
          assert(
            getExecutedPlan(df).count(
...
class BoltAggregateFunctionsDefaultSuite extends BoltAggregateFunctionsSuite {
  override protected def sparkConf: SparkConf = {
    super.sparkConf
      // Disable flush. This may cause spilling to happen on partial aggregations.
      .set(BoltConfig.BOLT_FLUSHABLE_PARTIAL_AGGREGATION_ENABLED.key, "false")
  }

  test("flushable aggregate rule") {
    withSQLConf(SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "false") {
      runQueryAndCompare(BoltAggregateFunctionsSuite.GROUP_SETS_TEST_SQL) {
        df =>
...
class BoltAggregateFunctionsFlushSuite extends BoltAggregateFunctionsSuite {
  override protected def sparkConf: SparkConf = {
    super.sparkConf
      ...
      .set(BoltConfig.ABANDON_PARTIAL_AGGREGATION_MIN_PCT.key, "50")
  ...
  test("flushable aggregate rule - double sum when floatingPointMode is strict") {
    withSQLConf(
      BoltConfig.MAX_PARTIAL_AGGREGATION_MEMORY.key -> "100",
      BoltConfig.COLUMNAR_BOLT_RESIZE_BATCHES_SHUFFLE_INPUT.key -> "false",
      GlutenConfig.COLUMNAR_MAX_BATCH_SIZE.key -> "2",
      BoltConfig.FLOATING_POINT_MODE.key -> "strict"
    ) {
      withTempView("t1") {
        import testImplicits._
  ...
  test("flushable aggregate rule - double sum when floatingPointMode is loose") {
    withSQLConf(
      BoltConfig.FLOATING_POINT_MODE.key -> "loose"
    ) {
      withTempView("t1") {
        import testImplicits._
...
class BoltAggregateFunctionsCompositeVectorSuite extends BoltAggregateFunctionsSuite {
  override protected def sparkConf: SparkConf = {
    super.sparkConf
      .set("spark.gluten.sql.columnar.backend.bolt.hashAggregationCompositeOutputEnabled", "true")
      .set("spark.gluten.sql.columnar.backend.bolt.hashAggregationUniqueRowOptEnabled", "true")
      .set(
...
object BoltAggregateFunctionsSuite {

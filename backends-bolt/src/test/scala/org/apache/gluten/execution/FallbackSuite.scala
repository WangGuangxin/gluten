...
class FallbackSuite extends BoltWholeStageTransformerSuite with AdaptiveSparkPlanHelper {
  ...
  private def collectColumnarToRow(plan: SparkPlan): Int = {
    collect(plan) { case v: BoltColumnarToRowExec => v }.size
  }
  ...
  test("fallback final aggregate of collect_list") {
    withSQLConf(
      ...
      val query =
        "SELECT sum(ele) FROM (SELECT c1, element_at(array_sort(collect_list(c2)), 1) as ele " +
          "FROM tmp3 GROUP BY c1)"
      runQueryAndCompare(
        query) {
        df =>
          val columnarToRow = collectColumnarToRow(df.queryExecution.executedPlan)
  ...
  // Elements in bolt_collect_set's output set may be in different order. This is a benign bug

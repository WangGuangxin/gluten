...
class ArrowEvalPythonExecSuite extends WholeStageTransformerSuite {
  ...
  private def pythonTest(testName: String)(testFun: => Any): Unit = {
    ignore(testName) {}
  }
  ...
  pythonTest("arrow_udf test: without projection") {
  ...
  pythonTest("arrow_udf test: with unrelated projection") {
    val pyarrowTestUDF = TestScalarPandasUDF(name = "pyarrowUDF")
    lazy val base =
      Seq(("1", 1), ("1", 2), ("2", 1), ("2", 2), ("3", 1), ("3", 2), ("0", 1), ("3", 0))
        .toDF("a", "b")
  ...
  pythonTest("arrow_udf test: with preprojection") {
    val pyarrowTestUDF = TestScalarPandasUDF(name = "pyarrowUDF")
    lazy val base =
      Seq(("1", 1), ("1", 2), ("2", 1), ("2", 2), ("3", 1), ("3", 2), ("0", 1), ("3", 0))
        .toDF("a", "b")

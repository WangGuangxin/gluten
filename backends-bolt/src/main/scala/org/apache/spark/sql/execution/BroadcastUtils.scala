...
// Utility methods to convert Vanilla broadcast relations from/to Bolt broadcast relations.
// FIXME: Truncate output with batch size.
object BroadcastUtils {
  def boltToSparkUnsafe[F, T](
    ...
    mode match {
      ...
      case IdentityBroadcastMode =>
        ...
        val toRelation = TaskResources.runUnsafe {
          ...
          /**
           * [[org.apache.gluten.execution.BoltColumnarToRowExec.toRowIterator()]] creates a single
  ...
  def sparkToBoltUnsafe[F, T](
      context: SparkContext,
      mode: BroadcastMode,
      schema: StructType,
      from: Broadcast[F],
      fn: Iterator[InternalRow] => Iterator[ColumnarBatch]): Broadcast[T] = {
    val useOffheapBuildRelation = BoltConfig.get.enableBroadcastBuildRelationInOffheap
    mode match {
      case HashedRelationBroadcastMode(_, _) =>
  ...
  def serializeStream(batches: Iterator[ColumnarBatch]): ColumnarBatchSerializeResult = {
    ...
    val values = filtered
      ...
      .toArray
    if (values.nonEmpty) {
      new ColumnarBatchSerializeResult(false, numRows, values.toSeq.asJava)
    } else {

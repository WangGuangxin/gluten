...
class BoltIteratorApi extends IteratorApi with Logging {
  ...
  override def genSplitInfo(
    ...
    val inputPartitions = partitions.map {
      case p: InputPartition => p
      case p =>
  ...
  private def constructSplitInfo(
    ...
    files.foreach {
      file =>
        ...
        (file.fileSize, file.modificationTime) match {
          case (size, time) if size >= 0 && time >= 0 =>
        ...
        metadataColumns.add(mapAsJavaMap(metadataColumn))
  ...
  override def genFirstStageIterator(
    ...
    val resIter: ColumnarBatchOutIterator =
      transKernel.createKernelWithBatchIterator(
        wsContext.root.toProtobuf.toByteArray,
        splitInfoByteArray,
        toColumnarBatchInIterators(inputIterators).toArray(new Array[ColumnarBatchInIterator](0)),
        partitionIndex,

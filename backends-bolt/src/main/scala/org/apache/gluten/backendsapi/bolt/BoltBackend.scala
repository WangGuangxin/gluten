...
object BoltBackendSettings extends BackendSettingsApi {
  ...
  override def validateScanExec(
    ...
    def validateEncryption(): Option[String] = {
      ...
      val metadataValidationEnabled = GlutenConfig.get.parquetMetadataValidationEnabled
    ...
    def validateFormats(): Option[String] = {
      ...
      distinctFileFormats.iterator
        .foldLeft(Option.empty[String]) {
          (acc, currentFormat) => if (acc.isDefined) acc else validateFormat(currentFormat)
        }
    }
  ...
  def supportNativeWrite(fields: Array[StructField]): Boolean = {
  ...
  def supportExpandExec(): Boolean = true
  ...
  def broadcastNestedLoopJoinSupportsFullOuterJoin(): Boolean = true

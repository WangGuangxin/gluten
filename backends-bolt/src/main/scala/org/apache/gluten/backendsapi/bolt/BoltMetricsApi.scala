...
class BoltMetricsApi extends MetricsApi with Logging {
  ...
  override def genColumnarShuffleExchangeMetrics(
    ...
    val baseMetrics = Map(
      ...
      "flattenTime" -> SQLMetrics.createNanoTimingMetric(
        sparkContext,
        "totaltime to flatten vector"),
      "useRowBased" -> SQLMetrics.createMetric(sparkContext, "number of tasks use Rowbased shuffle")
    )
  ...
  override def genWriteFilesTransformerMetrics(
      sparkContext: SparkContext): Map[String, SQLMetric] =
    Map(

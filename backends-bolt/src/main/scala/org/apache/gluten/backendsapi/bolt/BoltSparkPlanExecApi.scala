...
class BoltSparkPlanExecApi extends SparkPlanExecApi {
    ...
    if (useOffheapBroadcastBuildRelation) {
      TaskResources.runUnsafe {
        UnsafeColumnarBuildSideRelation(
          child.output,
          serialized.flatMap(_.onHeapData().asScala),
          mode)
      }
    } else {
      ColumnarBuildSideRelation(child.output, serialized.flatMap(_.onHeapData().asScala), mode)
    }
  }
  ...
  override def genWindowFunctionsNode(
    ...
    throw new GlutenNotSupportException("Window functions are not supported in Bolt backend yet")
  }

 ...
 */
case class BoltBloomFilterAggregate(
    child: Expression,
  ...
  override def prettyName: String = "bolt_bloom_filter_agg"
  ...
  override protected def withNewChildrenInternal(
      ...
      newNumBitsExpression: Expression): BoltBloomFilterAggregate = {
    copy(
      child = newChild,
  ...
  override def createAggregationBuffer(): BloomFilter = {
    if (!TaskResources.inSparkTask()) {
      throw new UnsupportedOperationException("bolt_bloom_filter_agg is not evaluable on Driver")
    }
    BoltBloomFilter.empty(Math.toIntExact(estimatedNumItems))
  }

  override def update(buffer: BloomFilter, input: InternalRow): BloomFilter = {
    assert(buffer.isInstanceOf[BoltBloomFilter])
  ...
  override def merge(buffer: BloomFilter, input: BloomFilter): BloomFilter = {
    assert(buffer.isInstanceOf[BoltBloomFilter])
    assert(input.isInstanceOf[BoltBloomFilter])
    buffer.asInstanceOf[BoltBloomFilter].mergeInPlace(input)
  }
  ...
  override def serialize(buffer: BloomFilter): Array[Byte] = {
    assert(buffer.isInstanceOf[BoltBloomFilter])
    buffer.asInstanceOf[BoltBloomFilter].serialize()
  }

  override def deserialize(bytes: Array[Byte]): BloomFilter = {
    BoltBloomFilter.readFrom(bytes)
  }

  override def withNewMutableAggBufferOffset(newOffset: Int): BoltBloomFilterAggregate =
    copy(mutableAggBufferOffset = newOffset)

  override def withNewInputAggBufferOffset(newOffset: Int): BoltBloomFilterAggregate =

...
case class BroadcastHashJoinExecTransformer(
  ...
  override protected def doValidateInternal(): ValidationResult = {
    if (output.exists(_.dataType == TimestampType)) {
      return ValidationResult.failed(
        "Bolt broadcast hash join with TimestampType output is disabled to avoid a native " +
          "broadcast build-side crash"
      )
    }
  ...
  override def columnarInputRDDs: Seq[RDD[ColumnarBatch]] = {
    ...
    val broadcastRDD = BoltBroadcastBuildSideRDD(sparkContext, broadcast)

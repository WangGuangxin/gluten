...
import org.apache.gluten.utils.BoltBatchResizer
 ...
 */
case class BoltResizeBatchesExec(
  ...
  override protected def mapIterator(in: Iterator[ColumnarBatch]): Iterator[ColumnarBatch] = {
    BoltBatchResizer.create(minOutputBatchSize, maxOutputBatchSize, in.asJava).asScala
  }

  override def batchType(): Convention.BatchType = BoltBatchType

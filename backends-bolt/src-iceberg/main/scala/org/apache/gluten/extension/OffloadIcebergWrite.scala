...
import org.apache.gluten.execution.{BoltIcebergAppendDataExec, BoltIcebergOverwriteByExpressionExec, BoltIcebergOverwritePartitionsDynamicExec, BoltIcebergReplaceDataExec}
...
case class OffloadIcebergOverwrite() extends OffloadSingleNode {
  override def offload(plan: SparkPlan): SparkPlan = plan match {
    case r: OverwriteByExpressionExec =>
      BoltIcebergOverwriteByExpressionExec(r)
    case other => other
  }
}
...
case class OffloadIcebergOverwritePartitionsDynamic() extends OffloadSingleNode {
  override def offload(plan: SparkPlan): SparkPlan = plan match {
    case r: OverwritePartitionsDynamicExec =>
      BoltIcebergOverwritePartitionsDynamicExec(r)
    case other => other
  }
}

 ...
 */
package org.apache.gluten.utils.bolt
...
import org.apache.spark.sql.execution.adaptive.bolt.BoltAdaptiveQueryExecSuite
...
class BoltTestSettings extends BackendTestSettings {
  ...
  enableSuite[GlutenDateExpressionsSuite]
    ...
    // https://github.com/facebookincubator/bolt/pull/10563/files#diff-140dc50e6dac735f72d29014da44b045509df0dd1737f458de1fe8cfd33d8145
  ...
  enableSuite[GlutenJsonFunctionsSuite]
    .exclude("json_object_keys function")
    .exclude("json_tuple - do not truncate results")
    .exclude("json_tuple filter and group")
    .exclude("json_tuple select")
    // * in get_json_object expression not supported in bolt
    .exclude("SPARK-42782: Hive compatibility check for get_json_object")
    // Bolt does not support single quotes in get_json_object function.
  ...
  enableSuite[BoltAdaptiveQueryExecSuite]
  ...
  enableSuite[GlutenParquetEncodingSuite]
    // Bolt does not support rle encoding, but it can pass when native writer enabled.
  ...
  enableSuite[GlutenParquetIOSuite]
    // Bolt doesn't write file metadata into parquet file.
    ...
    // Bolt parquet reader not allow offset zero.
  ...
  enableSuite[GlutenParquetThriftCompatibilitySuite]
    .exclude("Gluten - Read Parquet file generated by parquet-thrift")
  ...
  enableSuite[GlutenSQLAggregateFunctionSuite]
    .exclude("Gluten - Return NaN or null when dividing by zero")
  ...
  enableSuite[GlutenInsertSuite]
    ...
    // Bolt supports default values for new records but it does not backfill the
  ...
  enableSuite[GlutenDataFrameAggregateSuite]
    ...
    .exclude(
      "zero moments", // [bolt does not return NaN]
      ...
      "SPARK-19471: AggregationIterator does not initialize the generated result projection" +
        " before using it",
      // Bolt's collect_list / collect_set are by design declarative aggregate so plan check
  ...
  enableSuite[GlutenDateFunctionsSuite]
    ...
    .exclude("unix_timestamp")
    .exclude("to_unix_timestamp")
    // Unsupported datetime format: specifier X is not supported by bolt.
    .exclude("to_timestamp with microseconds precision")
    // Legacy mode is not supported, assuming this mode is not commonly used.
    .exclude("SPARK-30668: use legacy timestamp parser in to_timestamp")
    // Legacy mode is not supported and bolt getTimestamp function does not throw
  ...
  enableSuite[GlutenFileBasedDataSourceSuite]
    ...
    // error msg from bolt is different & reader options is not supported, rewrite
    ...
    // exclude as original metric not correct when task offloaded to bolt
  ...
  enableSuite[GlutenStatisticsCollectionSuite]
    ...
    // The output byte size of Bolt is different
  ...
  override def getSQLQueryTestSettings: SQLQueryTestSettings = BoltSQLQueryTestSettings
}

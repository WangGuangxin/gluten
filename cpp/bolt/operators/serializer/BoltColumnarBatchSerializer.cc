...
void BoltColumnarBatchSerializer::append(const std::shared_ptr<ColumnarBatch>& batch) {
  ...
  if (serializer_ == nullptr) {
    // Using first batch's schema to create the Bolt serializer. This logic was introduced in
    ...
    serializer_ = serde_->createSerializer(rowType, numRows, arena_.get(), &options_);
  }

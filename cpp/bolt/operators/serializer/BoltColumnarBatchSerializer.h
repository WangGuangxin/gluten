...
class BoltColumnarBatchSerializer final : public ColumnarBatchSerializer {
 ...
 private:
  void materializeSerializedBuffer();

  std::shared_ptr<bytedance::bolt::memory::MemoryPool> boltPool_;
  std::unique_ptr<bytedance::bolt::StreamArena> arena_;
  std::unique_ptr<bytedance::bolt::BatchVectorSerializer> serializer_;
  std::vector<bytedance::bolt::RowVectorPtr> batches_;
  std::unique_ptr<BufferOutputStream> serializedBufferStream_;

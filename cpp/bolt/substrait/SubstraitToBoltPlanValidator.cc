...
#include "SubstraitToBoltPlanValidator.h"
...
bool SubstraitToBoltPlanValidator::flattenSingleLevel(const TypePtr& type, std::vector<TypePtr>& out) {
  if (!type->isRow()) {
...
bool SubstraitToBoltPlanValidator::flattenDualLevel(const TypePtr& type, std::vector<std::vector<TypePtr>>& out) {
  if (!type->isRow()) {
    LOG_VALIDATION_MSG("Type is not a RowType.");
    return false;
  }
...
bool SubstraitToBoltPlanValidator::validateRound(
...
bool SubstraitToBoltPlanValidator::validateExtractExpr(const std::vector<core::TypedExprPtr>& params) {
  if (params.size() != 2) {
...
bool SubstraitToBoltPlanValidator::validateRegexExpr(
...
bool SubstraitToBoltPlanValidator::validateScalarFunction(
    const ::substrait::Expression::ScalarFunction& scalarFunction,
    const RowTypePtr& inputType) {
  ...
  for (const auto& argument : scalarFunction.arguments()) {
    ...
    params.emplace_back(exprConverter_->toBoltExpr(argument.value(), inputType));
  }
  ...
  if (!useIcuRegex_ && kRegexFunctions.find(name) != kRegexFunctions.end()) {
...
bool SubstraitToBoltPlanValidator::isAllowedCast(const TypePtr& fromType, const TypePtr& toType) {
  // Currently cast is not allowed for various categories, code has a bunch of rules
  // which define the cast categories and if we should offload to bolt. Currently,
...
bool SubstraitToBoltPlanValidator::validateCast(
  ...
  core::TypedExprPtr input = exprConverter_->toBoltExpr(castExpr.input(), inputType);

  if (SubstraitToBoltPlanValidator::isAllowedCast(input->type(), toType)) {
    return true;
  }
...
bool SubstraitToBoltPlanValidator::validateIfThen(
...
bool SubstraitToBoltPlanValidator::validateSingularOrList(
...
bool SubstraitToBoltPlanValidator::validateExpression(
...
bool SubstraitToBoltPlanValidator::validate(const ::substrait::WriteRel& writeRel) {
...
bool SubstraitToBoltPlanValidator::validate(const ::substrait::FetchRel& fetchRel) {
...
bool SubstraitToBoltPlanValidator::validate(const ::substrait::TopNRel& topNRel) {
...
bool SubstraitToBoltPlanValidator::validate(const ::substrait::GenerateRel& generateRel) {
...
bool SubstraitToBoltPlanValidator::validate(const ::substrait::ExpandRel& expandRel) {
  ...
  for (const auto& fields : expandRel.fields()) {
    ...
    if (fields.has_switching_field()) {
      ...
      for (const auto& projectExpr : projectExprs) {
        ...
        if (rowType) {
          expressions.emplace_back(exprConverter_->toBoltExpr(projectExpr, rowType));
        }
      }
...
bool SubstraitToBoltPlanValidator::validate(const ::substrait::WindowRel& windowRel) {
  ...
  for (const auto& expr : groupByExprs) {
    auto expression = exprConverter_->toBoltExpr(expr, rowType);
  ...
  for (const auto& sort : sorts) {
    ...
    if (sort.has_expr()) {
      auto expression = exprConverter_->toBoltExpr(sort.expr(), rowType);
      auto exprField = dynamic_cast<const core::FieldAccessTypedExpr*>(expression.get());
      if (!exprField) {
...
bool SubstraitToBoltPlanValidator::validate(const ::substrait::WindowGroupLimitRel& windowGroupLimitRel) {
...
bool SubstraitToBoltPlanValidator::validate(const ::substrait::SetRel& setRel) {
  switch (setRel.op()) {
    case ::substrait::SetRel_SetOp::SetRel_SetOp_SET_OP_UNION_ALL: {
      ...
      if (!parseBoltType(extension, inputRowType) || !flattenDualLevel(inputRowType, childrenTypes)) {
...
bool SubstraitToBoltPlanValidator::validate(const ::substrait::SortRel& sortRel) {
...
bool SubstraitToBoltPlanValidator::validate(const ::substrait::ProjectRel& projectRel) {
  ...
  for (const auto& expr : projectExprs) {
    ...
    expressions.emplace_back(exprConverter_->toBoltExpr(expr, rowType));
  }
  // Try to compile the expressions. If there is any unregistered function or
  // mismatched type, exception will be thrown.
  exec::ExprSet exprSet(std::move(expressions), execCtx_.get());
...
bool SubstraitToBoltPlanValidator::validate(const ::substrait::FilterRel& filterRel) {
  ...
  expressions.emplace_back(exprConverter_->toBoltExpr(filterRel.condition(), rowType));
...
bool SubstraitToBoltPlanValidator::validate(const ::substrait::JoinRel& joinRel) {
  ...
  if (joinRel.has_post_join_filter()) {
    auto expression = exprConverter_->toBoltExpr(joinRel.post_join_filter(), rowType);
    exec::ExprSet exprSet({std::move(expression)}, execCtx_.get());
  }
  return true;
}

bool SubstraitToBoltPlanValidator::validate(const ::substrait::CrossRel& crossRel) {
...
bool SubstraitToBoltPlanValidator::validateAggRelFunctionType(const ::substrait::AggregateRel& aggRel) {
  ...
  for (const auto& smea : aggRel.measures()) {
    ...
    auto baseFuncName =
        SubstraitParser::mapToBoltFunction(SubstraitParser::getNameBeforeDelimiter(funcSpec), isDecimal, useIcuRegex_);
...
bool SubstraitToBoltPlanValidator::validate(const ::substrait::AggregateRel& aggRel) {
  ...
  if (aggRel.has_advanced_extension()) {
    ...
    if (extension.has_enhancement() &&
        (!parseBoltType(extension, inputRowType) || !flattenSingleLevel(inputRowType, types))) {
  ...
  // The supported aggregation functions. TODO: Remove this set when Presto aggregate functions in Bolt are not
...
bool SubstraitToBoltPlanValidator::validate(const ::substrait::ReadRel& readRel) {
  planConverter_->toBoltPlan(readRel);
  ...
  if (readRel.has_filter()) {
    ...
    int32_t inputPlanNodeId = 0;
    std::vector<std::string> names;
    names.reserve(boltTypeList.size());
    for (auto colIdx = 0; colIdx < boltTypeList.size(); colIdx++) {
      names.emplace_back(SubstraitParser::makeNodeName(inputPlanNodeId, colIdx));
    }

    auto rowType = std::make_shared<RowType>(std::move(names), std::move(boltTypeList));
    std::vector<core::TypedExprPtr> expressions;
    if (!validateExpression(readRel.filter(), rowType)) {
      return false;
    }
    expressions.emplace_back(exprConverter_->toBoltExpr(readRel.filter(), rowType));
    // Try to compile the expressions. If there is any unregistered function
    // or mismatched type, exception will be thrown.
    exec::ExprSet exprSet(std::move(expressions), execCtx_.get());
  }
...
bool SubstraitToBoltPlanValidator::validate(const ::substrait::Rel& rel) {
...
bool SubstraitToBoltPlanValidator::validate(const ::substrait::RelRoot& relRoot) {
...
bool SubstraitToBoltPlanValidator::validate(const ::substrait::Plan& plan) {
  try {

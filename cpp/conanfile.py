...
# ============================================================================
# Gluten C++ conan recipe for the Gluten <-> Bolt joint build.
#
# Ported from PR #11261 (the original, working add_bolt_backend flow). It keeps
# the SAME conan coordinates the PR used:
#   * gluten requires  bolt/<BOLT_BUILD_VERSION>@<user>/<channel>
#     (bolt's own conan recipe -- exported by `make bolt-recipe` /
#      dev/builddeps-boltbe.sh bolt_recipe -- defines name="bolt",
#      cmake_target_name="bolt::bolt", cmake_file_name="bolt").
#   * the toolchain sets the ENABLE_BOLT cache variable, which makes
#     cpp/CMakeLists.txt include cpp/bolt.CMakeLists.cmake (PR #11261 pattern).
#
# DEVIATION FROM PR #11261 (the one intentional difference): the Gluten<->Bolt
# native bridge is GENERATED from cpp/velox at build time by dev/gen-bolt-cpp.sh
# (driven by cpp/bolt/CMakeLists.txt) instead of being a committed sed-renamed
# copy of cpp/velox. The produced backend library is libbolt.so (target `bolt`),
# loaded by the thin JVM BoltBackend (System.mapLibraryName("bolt")).
# ============================================================================
...
class GlutenConan(ConanFile):
    ...
    def generate(self):
        ...
        # Make cpp/CMakeLists.txt include cpp/bolt.CMakeLists.cmake (PR pattern).
    ...
    def package_info(self):
        ...
        # The codegen bridge builds the `bolt` target -> libbolt.so (cf. PR's
        # libbolt_backend; renamed to match the thin JVM loader which loads
        # System.mapLibraryName("bolt")).
        self.cpp_info.libs = ["bolt"]

        self.cpp_info.components["libgluten"].libs = ["gluten"]
        self.cpp_info.components["libgluten"].requires.append("bolt::bolt")

        self.cpp_info.components["bolt"].libs = ["bolt"]
        self.cpp_info.components["bolt"].requires.append("libgluten")
        self.cpp_info.components["bolt"].requires.append("bolt::bolt")

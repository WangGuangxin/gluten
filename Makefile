...
SHARED_LIBRARY ?= True
MVN_CMD ?= $(if $(wildcard $(ROOT_DIR)/build/mvn),$(ROOT_DIR)/build/mvn,mvn)
...
ifneq ($(wildcard $(JAVA_HOME)/bin/java),)
    ifneq ($(wildcard $(JAVA_HOME)/bin/javac),)
        ...
        ifneq ($(filter $(JDK_VERSION),$(ALLOWED_VERSIONS)),$(JDK_VERSION))
            $(error ERROR: JDK version $(JDK_VERSION) is not supported, only 8, 11 and 17 are allowed now)
        endif
    endif
endif
...
jar:
	...
	mv package/target/gluten-package-1.8.0-SNAPSHOT.jar output/gluten-spark3.2_2.12-1.0.0-SNAPSHOT-jar-with-dependencies.jar
...
jar_spark33:
	...
	mkdir -p output && \
	rm -rf output/gluten-spark*.jar
	mv package/target/gluten-package-1.8.0-SNAPSHOT.jar output/gluten-spark3.3_2.12-1.0.0-SNAPSHOT-jar-with-dependencies.jar

jar_spark34:
	java -version && mvn clean package -Pbackends-bolt -Pspark-3.4 -Pceleborn -Piceberg -DskipTests -Denforcer.skip=true -Ppaimon && \
	mkdir -p output && \
	rm -rf output/gluten-spark*.jar
	mv package/target/gluten-package-1.8.0-SNAPSHOT.jar output/gluten-spark3.4_2.12-1.0.0-SNAPSHOT-jar-with-dependencies.jar
...
jar_spark35:
	...
	mkdir -p output && \
	rm -rf output/gluten-spark*.jar
	mv package/target/gluten-package-1.8.0-SNAPSHOT.jar output/gluten-spark3.5_2.12-1.0.0-SNAPSHOT-jar-with-dependencies.jar

test:
	mvn -Pbackends-bolt -Pspark-3.2 -Pceleborn -Ppaimon package -Denforcer.skip=true

test_spark35:
	JAVA_HOME=/usr/lib/jvm/java-11-openjdk-amd64 $(MVN_CMD) clean package -Pjava-11 -Pbackends-bolt -Pspark-3.5 -Ppaimon -Phadoop-3.2 -Pceleborn -Piceberg -Denforcer.skip=true
